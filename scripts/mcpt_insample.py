...
def _fomc_hourly(wide, entry: str, exit_start: str) -> pd.DataFrame:
    ...
    idx = close.index
    # Session clock lives in ET; ClickHouse returns NAIVE-UTC timestamps and
    # snapshots return tz-aware UTC — both must convert (the EXP-46 bug, twice).
    et = (idx.tz_localize("UTC") if idx.tz is None else idx).tz_convert("America/New_York")
