...
def parse_symbols(value: str) -> list[str]:
    symbols = sorted({token.strip().upper() for token in (value or "").split(",") if token.strip()})
    if not symbols:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--symbols", required=True, help="Comma-separated underlyings, e.g. AAPL,MSFT")
    parser.add_argument("--strike-count", type=int, default=20, help="Strikes above/below ATM (default: 20)")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Fetch and parse, but do not write Iceberg")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return asyncio.run(run_snapshot(symbols=symbols, request_params=params, dry_run=args.dry_run))
