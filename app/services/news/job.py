...
async def run_news_ingest_once() -> dict:
    """One ingest+enrich cycle. Returns counts. Auditing is applied by the
    caller: the loop wraps this in ``audit_run(frequent=True)`` so no-op polls
    don't flood the runs log, while the manual-run path audits unconditionally."""
    ...
    # Concise logging: only emit an INFO line when something actually happened;
    # a quiet poll (no new filings / nothing enriched) stays at DEBUG.
    did_work = bool(ingest.stored or fomc_stored or econ_releases or enrich.enriched)
    logger.log(
        logging.INFO if did_work else logging.DEBUG,
        "news_ingest: stored=%d (fetched=%d matched=%d) fomc=%d econ=%d | enriched=%d/%d failed=%d",
...
async def run_news_ingest_loop() -> None:
    ...
    while True:
        try:
            async with audit_run("news_ingest", frequent=True) as rec:
