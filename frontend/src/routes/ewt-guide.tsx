...
/**
 * Elliott Wave Quick Guide — self-contained educational reference.
 *
 * Covers the three hard rules, all motive + corrective structures,
 * wave personalities, Fibonacci relationships, and modern market notes.
 * No API calls — purely static content derived from EWT first principles
 * and the elliottwave-forecast.com reference.
 */
export function EwtGuidePage() {
    ...
    <div className="mx-auto max-w-3xl space-y-10 px-4 py-8 md:px-6">
      ...
      <header className="space-y-1">
        ...
        <p className="text-sm text-fg-subtle">
          Ralph Nelson Elliott (1938) observed that market prices move in repetitive
          fractal wave patterns — 5 waves with the trend, 3 waves against it. Every
          timeframe nests inside a larger degree: a 5-minute impulse is a single wave
          inside an hourly structure that is itself one wave inside a daily count.
        </p>
      </header>

      {/* ── 1. The Three Inviolable Rules ─────────────────────────────── */}
      <Section title="1 · The Three Hard Rules" subtitle="Break any of these and the count is invalid — discard it.">
        <div className="grid gap-3 sm:grid-cols-3">
          <RuleCard
            number="Rule 1"
            title="Wave 2 never fully retraces Wave 1"
            body="Wave 2 cannot end at or below the origin of Wave 1 (in an uptrend). If it does, the count is wrong — what you labeled Wave 1 is not Wave 1."
            color="up"
          />
          <RuleCard
            number="Rule 2"
      ...
      {/* ── 2. Basic Structure ─────────────────────────────────────────── */}
      <Section title="2 · Basic Structure" subtitle="Every trend subdivides 5-3: five motive waves followed by three corrective waves.">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-md border border-border bg-bg-subtle p-4">
            <h3 className="mb-2 text-sm font-semibold text-fg-base">Motive (5-wave impulse)</h3>
  ...
  [0]   [2]  [5]
            ...
            <ul className="mt-3 space-y-1 text-xs text-fg-subtle">
              <li><Dot color="up" /> Waves 1, 3, 5 move <em>with</em> the trend</li>
              <li><Dot color="down" /> Waves 2, 4 are corrective retracements</li>
              <li><Dot color="accent" /> Wave 3 is typically longest &amp; strongest</li>
            </ul>
          </div>
          <div className="rounded-md border border-border bg-bg-subtle p-4">
            <h3 className="mb-2 text-sm font-semibold text-fg-base">Corrective (3-wave ABC)</h3>
            <pre className="font-mono text-xs leading-relaxed text-fg-muted">{`
  [B]
 /   \\
[A]  [C]
`.trim()}</pre>
            <ul className="mt-3 space-y-1 text-xs text-fg-subtle">
              <li><Dot color="down" /> A &amp; C move <em>against</em> the higher-degree trend</li>
              <li><Dot color="up" /> B is a counter-correction (partial recovery)</li>
              <li><Dot color="accent" /> Corrective patterns have far more variety than impulses</li>
            </ul>
          </div>
        </div>
      </Section>
      ...
      <Section title="3 · Wave Personalities" subtitle="Each wave has a characteristic sentiment, volume, and news backdrop. Recognizing these helps confirm counts in real time.">
      ...
      {/* ── 4. Motive Structures ──────────────────────────────────────── */}
      <Section title="4 · Motive Structures" subtitle="Structures that move in the direction of the one-larger-degree trend.">
      ...
      <Section title="5 · Corrective Structures" subtitle="Structures that move against the one-larger-degree trend. Corrective patterns have far more variety — never label a correction until the move is complete.">
        <div className="space-y-3">
          <StructureCard
            name="Zigzag"
            sub="A(5)-B(3)-C(5)"
          ...
          <StructureCard
            ...
            badgeColor="accent"
            points={[
              "Five corrective waves with converging trendlines — an ascending, descending, or symmetric wedge.",
      ...
      <Section title="6 · Fibonacci Relationships" subtitle="Fibonacci ratios define the most probable reversal zones and extension targets. These are guidelines — not rules. Expect clusters of ratios for high-confidence levels.">
        ...
        <p className="mt-2 text-xs text-fg-subtle">
          <span className="inline-block rounded-sm bg-accent/20 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-accent">highlighted</span>
          {" "} = most common / ideal ratio. Others are valid but less frequent.
        </p>
      </Section>

      {/* ── 7. Wave Degrees ───────────────────────────────────────────── */}
      <Section title="7 · Wave Degrees" subtitle="Elliott identified 9 degrees, from multi-decade Grand Super Cycles down to minute-level subminuette waves. Each degree nests cleanly inside the one above it.">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {DEGREES.map((d) => (
            <div key={d.name} className="rounded-md border border-border bg-bg-subtle px-3 py-2">
        ...
        <p className="mt-2 text-xs text-fg-subtle">
          This engine operates across degrees 0–3, using k-pivot windows of 4, 8, 16, and 32 bars to synthesize multi-degree counts simultaneously.
        </p>
      </Section>

      {/* ── 8. Modern Market Notes ────────────────────────────────────── */}
      <Section title="8 · Modern Market Notes" subtitle="Elliott developed his theory in the 1930s. Algorithmic trading has changed market microstructure in ways that require updated interpretation.">
        <div className="space-y-2">
          {MODERN_NOTES.map((n) => (
            <div key={n.title} className="rounded-md border border-border bg-bg-subtle px-3 py-2.5">
      ...
      {/* ── 9. Engine Coverage ────────────────────────────────────────── */}
      <Section title="9 · What This Engine Implements" subtitle="Coverage map for the StockAlert Elliott Wave engine (ew3.9.0).">
        <div className="grid gap-2 sm:grid-cols-2">
          {ENGINE_COVERAGE.map((item) => (
            ...
            >
              <span className={cn(
                "mt-0.5 flex-shrink-0 rounded-sm px-1.5 py-0.5 text-[9px] font-bold uppercase",
                item.status === "yes"
                  ? "bg-up/15 text-up"
                  : item.status === "partial"
              ...
              <div>
                ...
                {item.note ? <div className="text-[10px] text-fg-subtle">{item.note}</div> : null}
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className="border-t border-border pt-6 text-xs text-fg-subtle">
        ...
        <em>Elliott Wave Principle</em> (1978); elliottwave-forecast.com.
        Fibonacci relationships and wave personality descriptions are EWT guidelines
        (probabilistic), not rules. Always confirm counts with price action and
        momentum indicators.
      </footer>
    </div>
  );
...
function RuleCard({
  ...
  return (
    <div className={cn("rounded-md border p-3", cls)}>
      <span className={cn("mb-2 inline-block rounded-sm px-1.5 py-0.5 text-[10px] font-bold uppercase", badge)}>
        {number}
      </span>
...
function PersonalityRow({
  ...
  return (
    <div className="flex items-start gap-3 rounded-md border border-border bg-bg-subtle px-3 py-2.5">
      <span className={cn("flex-shrink-0 rounded-sm px-2 py-0.5 font-mono text-xs font-bold", badge)}>
        {wave}
      </span>
  ...
  return (
    <div className="rounded-md border border-border bg-bg-subtle p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className={cn("ml-auto rounded-sm px-1.5 py-0.5 text-[10px] uppercase", badgeCls)}>
          {badge}
        </span>
      </div>
      <ul className="mt-2 space-y-1">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-1.5 text-[11px] text-fg-subtle">
            <span className="mt-0.5 flex-shrink-0 text-[8px] text-fg-muted">▸</span>
            {p}
          </li>
        ))}
      </ul>
...
const WAVE_PERSONALITIES = [
  ...
  {
    wave: "W3",
    sentiment: "Conviction grows. Crowd begins participating at mid-wave. Gaps up often signal progression.",
  ...
  {
    wave: "W4",
    sentiment: "Frustrating sideways churn. Shallow and time-consuming relative to the move size.",
  ...
  {
    wave: "W5",
    sentiment: "Euphoric near the top. Average investors finally buy. Momentum diverges from price.",
  ...
  {
    wave: "WB",
    sentiment: "Bull trap. Looks like the trend resumed but fundamentals are no longer improving.",
  ...
  {
    wave: "WC",
    sentiment: "Bear market becomes obvious by Wave C's third leg. Capitulation and fear.",
...
const ENGINE_COVERAGE = [
  ...
  { label: "Leading diagonal", status: "partial" as const, note: "Detected same as ending; type distinction pending multi-degree context" },
